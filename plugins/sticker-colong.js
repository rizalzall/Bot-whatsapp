...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    try {
        ...
        if (/image|video/.test(mime)) {
            ...
            stiker = await sticker(img, false, senderName + ' ✅', 'Agar silahturahmi tidak terputus, izinkan saya ambil sticker us😁')
        } else if (args[0]) {
            let senderName = m.sender ? conn.getName(m.sender) : 'User'
            let url = args[0]
            ...
            stiker = await sticker(false, url, senderName + ' ✅', 'Agar silahturahmi tidak terputus, izinkan saya ambil sticker us😁')
        } else {
...
handler.command = /^((cl|colong)s|colong|maling|colongsticker|colongstiker|malingsticker|malingstiker)$/i
